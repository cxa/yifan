...
const PublicTimelineRoute = () => {
  ...
  useNativeHeaderTitle({ title: t('publicTimelineTitle') });
  ...
  const scrollHandler = useAnimatedScrollHandler({
    onScroll: event => { updatePullScrollY(event.contentOffset.y); },
  });
  ...
  const handleDeleteStatus = (status: FanfouStatus) => {
    ...
    return deleteStatus({
      ...
      t,
      onDeleted: () => {
        setTimelineItems(previous => previous.filter(item => getStatusId(item) !== statusId));
      },
    });
  };

  const fetchMore = async () => {
    if (isFetchingMore || isLoading || hasReachedTimelineEnd) return;
    const maxId = timelineItems.length > 0
    ...
    try {
      ...
      if (!hasAppended) setHasReachedTimelineEnd(true);
    } finally {
          ...
          ListEmptyComponent={
            <Animated.View style={timelineListSettings.animatedItemStyle}>
              {isLoading || isHydratingTimelineItems ? (
                ...
                <TimelineSkeletonCard message={t('publicTimelineEmpty')} />
              )}
            </Animated.View>
          }
...
export default PublicTimelineRoute;
